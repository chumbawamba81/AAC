...
import { Upload, Trash2, Link as LinkIcon, AlertCircle, CheckCircle2, Plus, RefreshCw } from 'lucide-react';
...
// === Feature flags ===
const ENABLE_LOCAL_MIGRATION = false; // diagnóstico/migração local desativado

// Tipos do estado legado (para ler docs locais se precisares)
...
export default function UploadDocsSection({ state, setState }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!error && data?.user && mounted) setUserId(data.user.id);
    })();
  ...
  const socioMissingCount = useMemo(() => {
    let miss = 0;
    for (const t of DOCS_SOCIO) if (!socioDocs.get(t)?.length) miss++;
    return miss;
  }, [socioDocs]);
  ...
  async function migrateLocal() {
    if (!userId) { alert('Sessão não encontrada.'); return; }
    setLoading(true);
    try {
      await migrateLocalDataUrls({ state, userId, onProgress: (m)=>console.log('[migrate]', m) });
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-8">
        ...
        {/* Diagnóstico/Migração local desativado por flag */}
        {ENABLE_LOCAL_MIGRATION && ((state.docsSocio && Object.keys(state.docsSocio).length > 0) ||
                        ...
                        <li key={row.id} className="flex items-center justify-between border rounded-md p-2">
                          <div className="text-sm flex items-center gap-2 min-w-0">
                            ...
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="underline inline-flex items-center gap-1 truncate max-w-[60vw] sm:max-w-[32rem]"
                              title={row.file_name || 'ficheiro'}
                            >
                              <LinkIcon className="h-4 w-4 shrink-0" />
                              <span className="truncate">{row.file_name || 'ficheiro'}</span>
                            </a>
                          </div>
