...
export default function AtletaFormCompleto({ initial, onSave, onCancel, dadosPessoais }: Props) {
  ...
  useEffect(()=>{
    if (a.dataNascimento && a.genero) setA(prev => ({ ...prev, escalao: computeEscalao(a.dataNascimento, a.genero) as any }));
  ...
  const isSub23 = a.escalao === 'Seniores masculinos Sub23 (2002-2007)'; // ajusta se necessário
  const showEscola = !!a.dataNascimento && !isMasters;

  const planoBloqueado = isMasters || isSub23;
  ...
  function save(ev: React.FormEvent) {
    ...
    // força "Anual" se escalão for Masters/Sub-23
    const planoEfetivo: PlanoPagamento = planoBloqueado ? 'Anual' : a.planoPagamento;
    const toSave = { ...a, planoPagamento: planoEfetivo };
  ...
  return (
    <form className="grid grid-cols-1 md:grid-cols-2 gap-4" onSubmit={save}>
      ...
      {/* Escalão (só leitura) e Plano juntos logo após Género */}
      ...
      >
        <select
          className={`input ${planoBloqueado ? 'bg-gray-100' : ''}`}
          value={planoBloqueado ? 'Anual' : a.planoPagamento}
        ...
        {planoBloqueado && <small className="text-gray-500">Para este escalão a anuidade é obrigatória.</small>}
      </Field>

      <Field label="Nacionalidade *">
        <select className="input" value={a.nacionalidade} onChange={e=>setA({...a, nacionalidade: e.target.value as Nacionalidade, nacionalidadeOutra: e.target.value==='Outra' ? a.nacionalidadeOutra : ''})}>
          ...
          <Field label="Encarregado de Educação *">
            <select
              ...
              onChange={e=>{
                ...
                setA(prev => ({ ...prev, encarregadoEducacao: v, parentescoOutro: v==='Outro' ? prev.parentescoOutro : '' }));
              }}
              required
            >
              <option value="">—</option>
      ...
      <Field className="md:col-span-2" label="Observações (opcional)">
        <textarea className="input min-h-[80px]" value={(a as any).observacoes || ''} onChange={e=>setA({ ...(a as any), observacoes: e.target.value })}/>
      </Field>
