...
type GateState =
  ...
  | {
      status: "denied";
      reason?: "not_authenticated" | "not_admin" | "error";
      message?: string;
    }
...
function AdminLogin({
  ...
  async function submit(ev: React.FormEvent) {
    ...
    try {
      ...
      if (!row)
        throw new Error(
          "Utilizador autenticado não consta na lista de administradores."
        );
      onLogged();
    } catch (e: any) {
    ...
    <div className="min-h-[60vh] flex items-center justify-center p-6">
      <Card>
        ...
        <CardContent className="p-4">
          {reasonMsg && (
            <p className="text-sm text-gray-600 mb-3">{reasonMsg}</p>
          )}
          <form className="space-y-3" onSubmit={submit}>
            ...
            <div className="flex items-center justify-between">
              ...
              <a
                href="/"
                className="inline-flex items-center justify-center p-5 text-base font-medium border border-default hover:border-default-medium text-body rounded-base bg-neutral-secondary-soft hover:text-heading hover:bg-neutral-tertiary"
              >
                <svg
                  aria-hidden="true"
                ...
                <svg
                  className="w-6 h-6 ms-1 rtl:rotate-180"
                  aria-hidden="true"
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
