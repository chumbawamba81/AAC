    ...
    <div className="space-y-3">
      ...
      {focus && (
        <AthleteDetailsDialog
          ...
          atleta={focus.atleta}// src/admin/components/AthletesTable.tsx
import React, { useEffect, useMemo, useState } from "react";
import { Download, RefreshCw, Search, Users, Eye } from "lucide-react";
import {
  listAtletasAdmin,
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.atleta.id} className="border-t">
                ...
                <Td>{r.titular?.tipo_socio || "—"}</Td>
                <Td>
                  <span className="inline-block rounded-full px-2 py-0.5 bg-gray-100 text-gray-800 text-xs">
                    {r.titular?.situacao_tesouraria || "—"}
                  </span>
                </Td>
                <Td>{r.missing ?? "—"}</Td>
